...
class errors(commands.Cog):
    ...
    async def app_command_errora(self, interaction, error):
        if isinstance(error, app_commands.MissingPermissions):
            embed = discord.Embed(title="Error", description="You are missing permission needed to run this command!",
            ...
            embed = discord.Embed(title="Error", description="I dont have permission for this to work!",
                                  color=discord.Color.red())
            embed.set_footer(text=f"{interaction.user.name}", icon_url=interaction.user.avatar.url)
            embed.timestamp = datetime.datetime.utcnow()
            try:
            ...
            embed = discord.Embed(title="Error",
                                  description=f"An error has occurred\nThis was recorded and sent to the devs!",
                                  color=discord.Color.red())
            embed.set_footer(text=f"{interaction.user.name}", icon_url=interaction.user.avatar.url)
            embed.timestamp = datetime.datetime.utcnow()
            try:
